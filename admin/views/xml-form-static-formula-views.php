...
if ($buttonid == $group.'_staticbutton_mapping'.$mapping) {
	...
	$static .= "<p style='margin-top:10px;border-radius:4px;border:1px solid #ddd;padding:10px;'>". __('HINT: Specify the CSV header to be added in between the curley braces({ }).','wp-advanced-importer')."<br /> ". __('Example: {post_title}','wp-advanced-importer')."</p>";
	$static .= "<a class='smack-btn mapping-close-btn' onclick=static_formula_divclose('".$group."','".$mapping."') style='float:right;color:red;'> ". __('Close','wp-advanced-importer'). " </a>";
	$static .="</div>";
	...
	$formula .= "<p style='margin-top:10px;border-radius:4px;border:1px solid #ddd;padding:7px;'>". __('HINT: Specify operator(+,-,*,/) and CSV header as operand in MATH() within {}.Example: MATH ({product_quantity}/{discount_price})','wp-advanced-importer')."</p>";
	$formula .= "<a class='smack-btn mapping-close-btn' style='float:right;color:red' onclick=static_formula_divclose('".$group."','".$mapping."')>". __('Close','wp-advanced-importer')."</a> </div>";
	print_r($formula);
	die;
}
else {
