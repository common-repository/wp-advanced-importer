...
if(!class_exists('WpXMLSmackUCIUploadHandler')) {
	class WpXMLSmackUCIUploadHandler {
		...
		protected function get_upload_path($file_name = null, $version = null) {
			...
			$eventDir = '';
			if($file_name) {
				$eventkey = $xml_uci_admin->convert_string2hash_key($file_name);
		...
		protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
			...
			if ($this->validate($uploaded_file, $file, $error, $index)) {
				...
				if($file_extension == 'csv' || $file_extension == 'txt'){
					$valid_csv = $xml_uci_admin->CheckCSV($file_path);
		...
		public function checkFileExists($fileName) {
			$file_contents = array();
			$file_with_path = XML_SM_UCI_IMPORT_DIR . '/' . $fileName;
	...
	new WpXMLSmackUCIUploadHandler();
}
